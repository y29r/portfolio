...
void main() {
	...
	if (particle.a >= 1.0) {
		...
		float normalDelta = uDeltaTime * 0.8;
		
		vec3 flowField = vec3(
			sin(snoise(vec4(particle.xyz, normalDelta)) * TAU + uTime),
			sin(snoise(vec4(particle.yxz, normalDelta)) * TAU + uTime),
			sin(snoise(vec4(particle.zxy, normalDelta)) * TAU + uTime)
		) * uPositionScale;

		if (disturbIntensity > 0.0){
			particle.xyz += flowField * disturbIntensity * uStrength * particle.a;
			particle.a += normalDelta;
		}
		else {
			particle.a += normalDelta;
		}
